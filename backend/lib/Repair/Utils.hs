...
import Repair.State (RepairState, getContexts, getEnvironments)
...
findDeclarationDiff ::
  ...
  Member (State RepairState) r =>
  Variable ->
  Sem r (Either (ΔLD.Diff Raw.Raw) (ΔGD.Diff Raw.Raw))
findDeclarationDiff v = do
  ...
  (Left <$> ΔLC.findLocalDeclarationDiff v γ δγ)
    `catch` ( \(localError :: String) ->
                (Right <$> ΔGE.findGlobalDeclarationDiff v e δe)
                  `catch` ( \(globalError :: String) ->
                              exc $
                                printf

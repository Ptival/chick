...
import Inductive.Inductive
  ( Constructor
    ...
    Inductive (inductiveConstructors, inductiveParameters),
    cpBinder,
    cpType,
    ipBinder,
    Φcp,
...
import Repair.Utils (Repair, RepairTermType'')
import Term.Binder (Binder (Binder))
-- import qualified Term.TypeChecked as C
import qualified Term.Raw as Raw
import Term.Term
  ( Branch (branchConstructor),
    GuardAndBody (GuardAndBody, branchBody),
    TermX (Hole),
    Variable,
    packBranch,
    unpackBranch,
...
repairBranch repairTerm b c@(Constructor _ _ cps _) δi δc = do
  ...
  δbody <-
    withStateFromConstructorArgs cpArgs δcpArgs cps δcps $
...
repairBranches repairTerm bs ind δi@(ΔI.Modify _ _ _ _ δcs) = do
  ...
  return $ foldr ($) ΔL.Same (permute pIndToMatch sortedDeltas ++ inserts)
  where
    go ::

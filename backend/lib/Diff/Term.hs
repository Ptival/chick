...
instance ToJSON α => ToJSON (Diff α)
      ...
      case (term, δterm) of
        ...
        (Lam a bt, CpyLam db dt) ->
          let (b, t') = unscopeTerm bt
           in Lam a <$> (abstractBinder <$> ΔA.patch b db <*> patch t' dt)
        ...
        (Pi a τ1 bτ2, CpyPi d1 db d2) ->
          let (b, τ2) = unscopeTerm bτ2
           in Pi a <$> patch τ1 d1 <*> (abstractBinder <$> ΔA.patch b db <*> patch τ2 d2)

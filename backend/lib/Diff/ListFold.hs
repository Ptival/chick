...
import Term.Term (Binder (..), TermX (Var), Variable)
import Utils (foldrWith, mapWithIndex, unzipMaybe)
  ...
  where
    ...
    onModify δa a _ acc =
      case patchA δa a of
        Nothing -> Nothing
        Just a' ->
          foldrWith (\_ acc -> ΔL.Remove <$> acc) (f a)
...
δListFoldMkAppVariables ::
  ΔListFold
    ...
    (Δ3.Diff (ΔA.Diff α) (ΔA.Diff Variable) (ΔT.Diff α))
    (ΔT.Diff α)
δListFoldMkAppVariables =
  ΔListFold
    { onInsert,

...
import Term.Term (TermX (App, Var), TypeX, Variable)
...
newtype GlobalEnvironment ξ ν = GlobalEnvironment
...
lookupDecl ::
  Eq ν =>
  ν ->
  GlobalEnvironment (C.Checked ν) ν ->
  Maybe (GlobalDeclaration (C.Checked ν) ν)
...
lookupRawType ::
  Variable ->
  GlobalEnvironment Raw.Raw Variable ->

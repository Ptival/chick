...
import Control.Lens (Field2 (_2), over)
...
extractSomeApps ::
  ...
  Int ->
  TermX α Variable ->
  Sem r (TermX α Variable, [(α, TermX α Variable)])
...
extractSomeApps _ t =
  let e :: String = printf "extractLams: not a Lam: %s" (show t)
   in throw e
...
extractSomeLams ::
  ...
  Int ->
  TermX α Variable ->
  Sem r ([(α, Binder Variable)], TermX α Variable)
...
extractSomePis _ t =
  let e :: String = printf "extractPis: not a Pi: %s" (show t)
   in throw e
...
extractPi = \case
  Pi a τ1 bτ2 -> do
    ...
    return (a, τ1, b, τ2)
  t ->
    let e :: String = printf "extractPi: not a Pi: %s" (show t)
     in throw e

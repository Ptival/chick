...
import Bound.Name (Name, abstract1Name, abstractName, instantiate1Name, instantiateName, name, _Name)
...
import Term.Binder (Binder (..), mkBinder)
...
instance ToJSON α => ToJSON (Branch α Variable) where
  toJSON b =
    let (ctor, parameters, body) = unpackBranch b
     in object
...
instance Bifunctor Branch where
  bimap :: forall a b c d. (a -> b) -> (c -> d) -> Branch a c -> Branch b d
...
instance Monad (TermX α) where
  return = Var Nothing
  (>>=) :: forall a b. TermX α a -> (a -> TermX α b) -> TermX α b
...
unscopeNamesWithT ::
  (Bound t, Foldable (t f), Monad f) =>
  (ν1 -> f ν2) ->
  ScopeT (Name ν1 Int) t f ν2 ->
  (Int -> Binder ν1, t f ν2)
unscopeNamesWithT f s =
  let varAtIndex = mkVarAtIndexT s
   in (Binder <$> varAtIndex, instantiateNameT (f . fromJust <$> varAtIndex) s)
...
unscopeNamesWith f s =
  let varAtIndex = mkVarAtIndex s
   in (Binder <$> varAtIndex, instantiateName (f . fromJust <$> varAtIndex) s)
...
instance Num (TermX () Variable) where
  ...
  fromInteger n
    | n < 0 = error "Num for Term only works for positive numbers"
    | n == 0 = Var Nothing "O"
